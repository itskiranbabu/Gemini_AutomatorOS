...
import { User, Shield, CreditCard, Bell, Key, Save, Activity, FileText } from 'lucide-react';
import { useAutomator } from '../store/AutomatorContext';
import { useToast } from '../store/ToastContext';

export const Settings = () => {
  const { profile, updateProfile, auditLogs } = useAutomator();
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 space-y-1">
          <button 
            ...
            className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg font-medium text-sm transition-all ${activeSection === 'general' ? 'bg-brand-600/10 text-brand-400 border-l-2 border-brand-500' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}
          >
            <User size={16} />
            <span>General</span>
          </button>
          <button 
            ...
            className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg font-medium text-sm transition-all ${activeSection === 'team' ? 'bg-brand-600/10 text-brand-400 border-l-2 border-brand-500' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}
          >
            <Shield size={16} />
           ...
           <button 
            ...
            className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg font-medium text-sm transition-all ${activeSection === 'audit' ? 'bg-brand-600/10 text-brand-400 border-l-2 border-brand-500' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}
          >
            <FileText size={16} />
        ...
        <div className="flex-1 space-y-6">
          ...
          {activeSection === 'audit' && (
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 h-[600px] overflow-hidden flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                      {auditLogs.length === 0 ? (
                          ...
                          auditLogs.map(log => (
                              <div key={log.id} className="p-3 bg-slate-950 rounded-lg border border-slate-800 flex items-center justify-between">
                                  <div>
                                      <div className="flex items-center space-x-2">
                                          <span className="text-xs font-bold text-brand-400 bg-brand-900/20 px-1.5 rounded">{log.action}</span>
                                          <span className="text-sm text-slate-300">{log.entityName || 'System'}</span>
                                      </div>
                                      <div className="text-xs text-slate-500 mt-0.5">{log.user} • {log.details}</div>
                                  </div>
